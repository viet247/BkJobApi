...
RSpec.describe "Api::V1::Jobs", type: :request do
  ...
  describe "Get /api/v1/jobs/:id" do
    # Happy path
    context "Job is exists" do
      it "Return status code 200" do
        get "/api/v1/jobs/#{jobs.first.id}"
        expect(response).to have_http_status(:ok)
      end
    end

    # Sad path
    context "Job is not exists" do
      before { get "/api/v1/jobs/-1" }
      it "Return status code 404" do
        expect(response).to have_http_status(:not_found)
      end
      it "Return right message in body" do
        json = JSON.parse(response.body)
        expect(json['message']).to eq("Job not found!")
      end
    end
  end
end
